...
def mark_run_succeeded(conn: Connection, *, run_id: UUID) -> None:
    ...
    set_run_status(conn, run_id=run_id, status="succeeded", error_message=None, set_finished_at=True)
def mark_run_failed(conn: Connection, *, run_id: UUID, error_message: str) -> None:
    """Mark a run as `"failed"`, and set `finished_at`."""
    set_run_status(conn, run_id=run_id, status="failed", error_message=error_message, set_finished_at=True)
